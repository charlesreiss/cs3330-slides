...
\newcommand{\markSub}{4}
\newcommand{\markXor}{5}
\newcommand{\markGlobal}{6}
\newcommand{\firstOCode}{7}
\newcommand{\firstOData}{8}
\newcommand{\firstSOCorrespond}{9}
\newcommand{\firstOReloc}{10}
\newcommand{\firstOSymbols}{11}
\newcommand{\firstOLStrNote}{12}
\newcommand{\firstExe}{13}
    \lstset{
...
\end{visibleenv}
    ...
    \begin{visibleenv}<\markGlobal>
        \draw[red,very thick,Latex-] ([yshift=-.7cm,xshift=-2.5cm]helloS.north east) coordinate (globalEnd) -- ++(1.75cm,0cm) node[right,font=\small,fill=white,draw] {mark used by other files};
    \end{visibleenv}

    \begin{visibleenv}<\markXor>
        \draw[red,very thick,Latex-] ([yshift=-2.25cm,xshift=-2cm]helloS.north east) coordinate (xorEnd) -- ++(1.75cm,0cm)
            coordinate (xorLast);
    ...
    \begin{visibleenv}<\markSub>
        \draw[red,very thick,Latex-] ([yshift=-1.25cm,xshift=-2.25cm]helloS.north east) coordinate (subEnd) -- ++(2cm,0cm)
            coordinate (subLast);
        \draw[red,very thick,Latex-] ([yshift=-2.5cm,xshift=-2.25cm]helloS.north east) coordinate (addEnd) -- ++(2cm,0cm)
            coordinate (addLast);
    ...
    \begin{visibleenv}<\firstOLStrNote>
        \draw[Latex-,red,very thick] ([yshift=-.7cm,xshift=-.3cm]helloOReloc.north east) -- ++(2cm, 0cm)
            node[right,font=\small,draw,fill=white,align=left] {
            \texttt{.Lstr} location specified w/o name \\
            and not usable by other files \\
            so no symbol table entry needed \\
            ~ \\
            (convention: .L\ldots labels always local)
        };
    \end{visibleenv}
