\title{introduction / layers of abstraction}
\date{}
\begin{document}
\begin{frame}
    \titlepage
\end{frame}

\usemintedstyle{tango}
\newmintinline{c}{}
\newminted[ccode]{c}{linenos=true,xleftmargin=1cm,beameroverlays,escapeinside=@@,texcomments}
\newminted[ccodeNL]{c}{linenos=false,beameroverlays,escapeinside=@@,texcomments}
\newminted[ccodeS]{c}{linenos=false,fontsize=\small,beameroverlays,escapeinside=@@,texcomments}
\newminted[ccodeT]{c}{linenos=false,fontsize=\scriptsize,beameroverlays,escapeinside=@@,texcomments}
\lstnewenvironment{asmcodeNL}{\lstset{language=myasm,escapeinside=@@,deletekeywords=test}}{}
\lstnewenvironment{asmcodeS}{\lstset{language=myasm,style=small,escapeinside=@@}}{}
\lstnewenvironment{asmcodeT}{\lstset{language=myasm,style=smaller,escapeinside=@@}}{}
\renewcommand{\theFancyVerbLine}{\small\textcolor[rgb]{.25,.25,.25}{\arabic{FancyVerbLine}}}

\usetikzlibrary{calc,positioning}

\input{../intro/lecture-logistics}

\begin{frame}
    \titlepage
\end{frame}

\input{../common/layers}
\input{../common/programMemory}
\input{../common/maybeEmph}

\section{Course Introduction}

\subsection{layers of abstraction}
\layersHighlighting{none}

\subsection{C}
\layersHighlighting{hll}

\input{../intro/whyC}

\input{../intro/getC}

\subsection{assembly}

\layersHighlighting{asm}

\input{../intro/asmKnown}

\subsection{machine code}
\layersHighlighting{machineCode}

\input{../intro/y86Blurb}

\subsection{what the processor does}
\layersHighlighting{hdl}
\input{../intro/textbook-aside}

\input{../intro/cpuPlusMemory}

\subsection{Goals (if you don't care about HW)}

\input{../intro/goals}
\section{Logistics}
\input{../intro/logistics}

\begin{frame}{quiz demo}
\end{frame}

\section{Memory and Endianness}

\input{../intro/endianness}

\subsection{exercise}
\input{../intro/endianness-ex} % FIXME: soln slide

\subsubsection{explanation}
\input{../intro/endianness-ex-explain} % FIXME: soln slide

\end{document}
