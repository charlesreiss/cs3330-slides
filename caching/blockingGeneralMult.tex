...
\end{lstlisting}
\begin{itemize}
    \item \myemph<2>{$C_{ij}$} used $K$ times for one cache load
    \item \myemph<3>{$A_{ik}$} used $J$ times for one cache load
    \item \myemph<4>{$B_{kj}$} used $I$ times for one cache load
